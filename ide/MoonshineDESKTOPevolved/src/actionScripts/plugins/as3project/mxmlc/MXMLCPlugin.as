	...
	public class MXMLCPlugin extends PluginBase implements IPlugin, ISettingsProvider
	{
		...
		public function getSettingsList():Vector.<ISetting>
		{
			return Vector.<ISetting>([
				new PathSetting(this,'defaultFlexSDK', 'Default Apache® Flex SDK', true, defaultFlexSDK, true),
		...
		private function shellData(e:ProgressEvent):void 
		{
			if(fcsh)
			{
				...
				{ // Successful compile
					...
					swfPath = (currentProject as AS3ProjectVO).swfOutput.path.fileBridge.nativePath;
					print("Project Build Successfully");
					dispatcher.dispatchEvent(
						new RefreshTreeEvent( (currentProject as AS3ProjectVO).swfOutput.path )
					);
		...
		private function testMovie(swfFilePath:String):void 
		{
			...
			if (pvo.resourcePaths.length != 0)
			{
				...
				for each (var i:FileLocation in pvo.resourcePaths)
				{
					(i.fileBridge.getFile as File).copyTo(destination.resolvePath(i.fileBridge.name), true);
				}
			}
