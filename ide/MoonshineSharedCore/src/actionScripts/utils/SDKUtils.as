...
package actionScripts.utils
{
	...
	public class SDKUtils extends EventDispatcher
	{
		...
		public static function openSDKUnzipPrompt():void
		{
			...
			if (ConstantsCoreVO.IS_BUNDLED_SDK_PRESENT && ((IDEModel.getInstance().userSavedSDKs.length == 0) || (IDEModel.getInstance().userSavedSDKs[0].status != SDKUtils.BUNDLED)) && !ConstantsCoreVO.IS_BUNDLED_SDK_PROMPT_DNS) GlobalEventDispatcher.getInstance().dispatchEvent(new Event(HelpPlugin.EVENT_SDK_UNZIP_REQUEST));
			else if (((IDEModel.getInstance().userSavedSDKs.length == 0) || (IDEModel.getInstance().userSavedSDKs[0].status != SDKUtils.BUNDLED)) && !ConstantsCoreVO.IS_SDK_HELPER_PROMPT_DNS) GlobalEventDispatcher.getInstance().dispatchEvent(new Event(HelpPlugin.EVENT_SDK_HELPER_DOWNLOAD_REQUEST));
			else GlobalEventDispatcher.getInstance().dispatchEvent(new Event(HelpPlugin.EVENT_CHECK_MINIMUM_SDK_REQUIREMENT)); 
		}
		...
		private static function getUserDownloadsSDKFolder():FileLocation
		{
			...
			var extractionDir:FileLocation = new FileLocation("/" + tmpUserFolderSplit.join("/") + "/Downloads/"+ EXTRACTED_FOLDER_NAME);
