...
package actionScripts.plugin.splashscreen
{
	...
	public class SplashScreenPlugin extends PluginBase implements IMenuPlugin, ISettingsProvider
	{
		...
		override public function activate():void
		{
			...
			if(!model.defaultSDK && ConstantsCoreVO.IS_AIR && (!ConstantsCoreVO.IS_BUNDLED_SDK_PRESENT || ConstantsCoreVO.IS_BUNDLED_SDK_PROMPT_DNS) && (!ConstantsCoreVO.IS_MACOS || ConstantsCoreVO.IS_SDK_HELPER_PROMPT_DNS))
			{
				setTimeout(model.noSDKNotifier.notifyNoFlexSDK, 1000);
			}
			else if (model.defaultSDK && ConstantsCoreVO.IS_AIR && (!ConstantsCoreVO.IS_BUNDLED_SDK_PRESENT || ConstantsCoreVO.IS_BUNDLED_SDK_PROMPT_DNS) && (!ConstantsCoreVO.IS_MACOS || ConstantsCoreVO.IS_SDK_HELPER_PROMPT_DNS))
			{
